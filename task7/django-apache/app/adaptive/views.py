...
def pdf_files(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            services.save_file_by_session_id(session_key, form.cleaned_data['file'])
    ...
    context = {
        'form': form,
        'files': services.get_files_by_session_id(session_key),
    }
...
def download_pdf_file(request, filename):
    session_key = get_or_create_session_key(request)
    path = services.get_file_by_session_id(session_key, filename)
